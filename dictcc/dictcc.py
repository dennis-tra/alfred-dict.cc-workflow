...
class Dict(object):
    ...
    def translate(cls, word, from_language, to_language):
        if any(map(lambda l: l not in AVAILABLE_LANGUAGES.keys(),
    ...
    def _get_response(cls, word, from_language, to_language):
        ...
        query_str = "http://"+subdomain+".dict.cc/?s="+formatted_word
    ...
    def _correct_translation_order(cls, result, word):
        ...
        def make_lowercase(str):
            return str.lower()

        [from_words, to_words] = zip(*result.translation_tuples)

        from_words_lowercase = map(make_lowercase, from_words)
        to_words_lowercase = map(make_lowercase, to_words)

        if from_words_lowercase.count(word.lower()) >= to_words_lowercase.count(word.lower()):
            return result
        else:
